...
const Team = () => {
  const [team, setTeam] = useState<TeamMember[]>(() => mergeTeamMembers(teamMembers, []));
        ...
        <Container>
          ...
          >
            <h2 className="section-title">
              Notre <span style={{ color: "var(--accent-logo-blue)" }}>Équipe</span>
            </h2>
          </motion.div>
          ...
          <div className="team-grid">
            {team.map((member, i) => {
              ...
              return (
                <Link key={member.slug || member.name} to={`/team/${member.slug}`} className="block">
                  <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    whileHover={{ y: -4 }}
