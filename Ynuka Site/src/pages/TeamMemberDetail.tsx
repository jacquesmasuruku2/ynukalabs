...
const TeamMemberDetail = () => {
  ...
  const [member, setMember] = useState<TeamMember | null>(
    () => teamMembers.find((item) => item.slug === slug) ?? null
  );
  ...
  useEffect(() => {
    const loadMember = async () => {
      ...
      try {
        ...
        const mapped = rows.map(mapAdminTeamMember).filter((m) => m.name && m.role);
        const merged = mergeTeamMembers(teamMembers, mapped);
        const found =
          merged.find((m) => m.slug === slug) ??
          merged.find((m) => slugifyTeamName(m.name) === slug) ??
          null;
        setMember(found);
      } catch {
        setMember(
          teamMembers.find((item) => item.slug === slug) ??
            teamMembers.find((item) => slugifyTeamName(item.name) === slug) ??
            null
        );
      }
    };
  ...
  return (
    <div className="min-h-screen bg-[var(--dark-bg)] py-16 text-white">
      <div className="container mx-auto max-w-4xl px-4">
        <Link to="/team" className="mb-8 inline-flex items-center gap-2 text-white/70 hover:text-white">
        ...
        <article className="grid gap-8 rounded-3xl bg-white/5 p-6 ring-1 ring-white/10 md:grid-cols-[240px_1fr] md:p-10">
          <div className="mx-auto h-56 w-56 overflow-hidden rounded-2xl bg-white/10">
            {member.image && !member.image.startsWith("TO_ADD_") ? (
              ...
              <Users className="m-20 h-16 w-16 text-white/50" />
            )}
          </div>
          <div>
            ...
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: member.description || "Aucune description disponible pour le moment.",
              }}
            />
            <div className="mt-8 flex gap-3">
              ...
              {member.social.x.startsWith("http") && (
                <a
                  ...
                  className="rounded-full bg-white/10 p-3"
                >
                  <XLogo weight="bold" className="h-5 w-5" />
                </a>
              )}
              ...
              {member.social.telegram.startsWith("http") && (
                <a
                  ...
                  className="rounded-full bg-white/10 p-3"
                >
                  <TelegramLogo weight="bold" className="h-5 w-5" />
                </a>
              )}
            </div>
          </div>
