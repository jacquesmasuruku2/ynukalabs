...
export async function sendAdminLoginAlertEmail({
  ...
  return transporter.sendMail({
    ...
    html: `<!doctype html><html lang="fr"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><style>@media screen and (max-width:600px){body{margin:0!important;padding:0!important}.security-shell{width:100%!important;max-width:100%!important;border-radius:0!important}.security-padding{padding:24px 16px!important}.security-details{table-layout:fixed!important;width:100%!important}.security-details td{display:block!important;width:100%!important;box-sizing:border-box!important;overflow-wrap:anywhere!important;word-break:break-word!important}.security-details td+td{padding-top:0!important;border-top:0!important}.security-details td:first-child{padding-bottom:3px!important}.security-details td:last-child{padding-top:0!important;padding-bottom:12px!important}.security-alert{font-size:14px!important;line-height:1.55!important}}</style></head><body style="margin:0;background:#f3f6fa;padding:24px;font-family:Arial,Helvetica,sans-serif;color:#14213d"><div class="security-shell" style="width:100%;max-width:600px;margin:0 auto;background:#fff;border:1px solid #dfe6ef;border-radius:14px;overflow:hidden"><div style="height:6px;background:#ffb800"></div><div class="security-padding" style="padding:30px"><p style="margin:0 0 18px;color:#0b3b8b;font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase">Ynuka Labs</p><h1 style="margin:0 0 16px;font-size:24px;line-height:1.25;color:#102a43">Nouvelle connexion détectée</h1><p style="font-size:16px;line-height:1.6">Bonjour ${safe(adminName)},</p><p style="font-size:15px;line-height:1.6">Une connexion vient d'être effectuée sur le panel d'administration.</p><table role="presentation" width="100%" cellpadding="0" cellspacing="0" class="security-details" style="margin:22px 0;border-collapse:collapse;font-size:14px;table-layout:fixed"><tr><td style="padding:10px 0;color:#65758b;width:38%">Compte</td><td style="padding:10px 0;font-weight:700;overflow-wrap:anywhere;word-break:break-word">${safe(adminEmail)}</td></tr><tr><td style="padding:10px 0;color:#65758b;border-top:1px solid #edf1f5">Date et heure (UTC+2)</td><td style="padding:10px 0;border-top:1px solid #edf1f5;overflow-wrap:anywhere;word-break:break-word">${safe(date)}</td></tr><tr><td style="padding:10px 0;color:#65758b;border-top:1px solid #edf1f5">Adresse IP</td><td style="padding:10px 0;border-top:1px solid #edf1f5;overflow-wrap:anywhere;word-break:break-word">${safe(ipAddress)}</td></tr><tr><td style="padding:10px 0;color:#65758b;border-top:1px solid #edf1f5">Localisation</td><td style="padding:10px 0;border-top:1px solid #edf1f5;overflow-wrap:anywhere;word-break:break-word">${safe(location)}</td></tr><tr><td style="padding:10px 0;color:#65758b;border-top:1px solid #edf1f5">Appareil</td><td style="padding:10px 0;border-top:1px solid #edf1f5;overflow-wrap:anywhere;word-break:break-word">${safe(device)}</td></tr><tr><td style="padding:10px 0;color:#65758b;border-top:1px solid #edf1f5">Navigateur</td><td style="padding:10px 0;border-top:1px solid #edf1f5;overflow-wrap:anywhere;word-break:break-word">${safe(userAgent)}</td></tr></table><p class="security-alert" style="margin:0;padding:14px 16px;border-left:4px solid #e56a6a;background:#fff3f3;color:#6b2b2b;font-size:14px;line-height:1.6">Si cette connexion ne vient pas de vous, changez immédiatement votre mot de passe et contactez l'équipe Ynuka Labs.</p></div></div></body></html>`,
  });
}
...
export async function sendNewsletterEmail({
  ...
  if (!host || !user || !pass) throw new Error('SMTP configuration is missing');
  const displayName = (name || '').trim() || 'abonné(e)';
  const personalize = (value: string) => value.replace(/\{\{name\}\}/gi, displayName);
  return transporter.sendMail({
    from,
    to,
