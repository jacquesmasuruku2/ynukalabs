...
export default function NewGalleryEventPage() {
  ...
  const [form, setForm] = useState({ title: '', subtitle: '', date: '', description: '', imagesText: '' });
    ...
    <AdminLayout>
      <div className="mx-auto max-w-2xl space-y-6">
        ...
        <form onSubmit={submit} className="card space-y-4 rounded-lg border p-6">
          ...
          <label className="block text-sm font-medium text-secondary">URLs images (une par ligne)<textarea value={form.imagesText} onChange={(e) => update('imagesText', e.target.value)} rows={6} className="mt-2 w-full rounded-md border px-3 py-2 font-mono text-sm" placeholder="https://..." /></label>
