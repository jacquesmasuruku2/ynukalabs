...
export default function NewPartnerPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    setSaving(false);
    if (res.ok) router.push('/partners'); else alert('Création impossible.');
  };
    ...
    <AdminLayout>
      <div className="mx-auto max-w-2xl space-y-6">
        <div className="flex items-center justify-between">
          <h1 className="text-3xl font-bold text-primary">Nouveau partenaire</h1>
        ...
        <form onSubmit={submit} className="card space-y-4 rounded-lg border p-6">
          <label className="block text-sm font-medium text-secondary">Nom *<input required value={form.name} onChange={(e) => { update('name', e.target.value); if (!form.slug) update('slug', slugify(e.target.value)); }} className="mt-2 w-full rounded-md border px-3 py-2" /></label>
